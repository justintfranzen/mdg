...
<?php
$gated_content_page_toggle = get_field('gated_content_page_toggle');
$redirect_url = 'http://mdgbio.test/wp-login.php';
...
?>
  ...
  <?php if ($gated_content_page_toggle == true) {
    if (is_user_logged_in()) {
      echo '<!-- User is Logged in -->';
    } else {
      $redirect_url = isset($redirect_url['url']) ? $redirect_url['url'] : home_url();
		...
		<div id="top-header"<?php echo $et_top_info_defined ? '' : 'style="display: none;"'; ?>>
			...
			<?php if ($et_contact_info_defined): ?>
				...
				<?php if (!empty(($et_phone_number = et_get_option('phone_number')))): ?>
					<span id="et-info-phone"><?php echo et_core_esc_previously(
       et_sanitize_html_input_text(strval($et_phone_number)),
     ); ?></span>
				<?php endif; ?>

				<?php if (!empty(($et_email = et_get_option('header_email')))): ?>
					<a href="<?php echo esc_attr('mailto:' . $et_email); ?>"><span id="et-info-email"><?php echo esc_html(
  $et_email,
); ?></span></a>
				<?php endif; ?>
...
?>
	...
	<?php ob_start(); ?>
		<header id="main-header" data-height-onload="<?php echo esc_attr(et_get_option('menu_height', '66')); ?>">
			...
			<?php
   $logo =
     ($user_logo = et_get_option('divi_logo')) && !empty($user_logo)
       ? $user_logo
       : $template_directory_uri . '/images/logo.png';

   // Get logo image size based on attachment URL.
   $logo_size = et_get_attachment_size_by_url($logo);
   $logo_width = !empty($logo_size) && is_numeric($logo_size[0]) ? $logo_size[0] : '93'; // 93 is the width of the default logo.
   $logo_height = !empty($logo_size) && is_numeric($logo_size[1]) ? $logo_size[1] : '43'; // 43 is the height of the default logo.
   ...
   ?>
				<div class="logo_container">
					...
					<a href="<?php echo esc_url(home_url('/')); ?>">
						<img src="<?php echo esc_attr($logo); ?>" width="<?php echo esc_attr($logo_width); ?>" height="<?php echo esc_attr(
  $logo_height,
); ?>" alt="<?php echo esc_attr(get_bloginfo('name')); ?>" id="logo" data-height-percentage="<?php echo esc_attr(
  et_get_option('logo_height', '54'),
); ?>" />
					</a>
				</div>
			<?php
   ...
   ?>
				<div id="et-top-navigation" data-height="<?php echo esc_attr(
      et_get_option('menu_height', '66'),
      ...
      if (empty($primaryNav)): ?>
           ...
           echo 'class="current_page_item"';
						...
						<?php else:echo et_core_esc_wp($primaryNav);endif;
      ?>
						</nav>
					<?php endif; ?>
       ...
       et_show_cart_total([
         'no_text' => true,
					...
					<?php if ($et_slide_header || is_customize_preview()): ?>
						<span class="mobile_menu_bar et_pb_header_toggle et_toggle_<?php echo esc_attr(
        et_get_option('header_style', 'left'),
      ); ?>_menu"></span>
					<?php endif; ?>
